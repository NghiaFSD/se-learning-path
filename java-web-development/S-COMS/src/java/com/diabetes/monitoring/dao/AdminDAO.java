...
public class AdminDAO {
    ...
    public int autoTransitionAppointmentStatuses() {
        ...
        try (Connection connection = DatabaseConnection.getConnection()) {
            ...
            // Step 1: Waiting -> In_Progress nếu đã đến giờ
            ...
            try (PreparedStatement stmt1 = connection.prepareStatement(sql1)) {
                ...
                if (updated1 > 0) {
                    LOGGER.log(Level.INFO, "[DEMO] Auto-transitioned Waiting -> In_Progress: {0} appointments", updated1);
                }
            }

            // Step 2: In_Progress -> Completed nếu hết ca (2 giờ) và có hồ sơ bệnh nhân
            String sql2 = "UPDATE Appointment "
                    + "SET status = 'Completed' "
                    + "WHERE LOWER(status) IN ('in_progress', 'in-progress') "
                    + "AND DATEADD(HOUR, 2, appointment_time) <= GETDATE() "
                    + "AND patient_id IS NOT NULL "
                    + "AND EXISTS ("
                    + "    SELECT 1 FROM Healthy_Record hr "
                    + "    WHERE hr.patient_id = Appointment.patient_id"
                    + ")";
            try (PreparedStatement stmt2 = connection.prepareStatement(sql2)) {
                int updated2 = stmt2.executeUpdate();
                totalUpdated += updated2;
                if (updated2 > 0) {
                    LOGGER.log(Level.INFO, "[DEMO] Auto-transitioned In_Progress -> Completed: {0} appointments", updated2);
                }
            }
    ...
    public List<Map<String, Object>> getDoctorsForAiScheduling(Date startDate, Date endDate) {
        ...
        String sql = "SELECT d.doctor_id, d.full_name, d.department, LOWER(a.status) AS account_status, "
                ...
                + "      AND ap.status IN ('Waiting', 'In_Progress') "
        ...
        try (Connection connection = DatabaseConnection.getConnection()) {
            ...
            try {
                ...
                for (Map<String, Object> assignment : assignments) {
                    ...
                    String department = String.valueOf(assignment.get("department"));
    ...
    public List<Map<String, Object>> createAiOptimizedSchedules(List<Date> targetDates,
        ...
        String pickDoctorSql = "SELECT TOP 1 d.doctor_id, d.full_name, d.department, "
                ...
                + "   SELECT ds.doctor_id, COUNT(ap.appointment_id) AS active_count "
                + "   FROM Doctor_Schedule ds "
                + "   LEFT JOIN Appointment ap ON ap.schedule_id = ds.schedule_id AND ap.status IN ('Waiting', 'In_Progress') "
                + "   WHERE ds.work_date BETWEEN ? AND ? "
                + "   GROUP BY ds.doctor_id"
                + ") active_load ON active_load.doctor_id = d.doctor_id "
                + "LEFT JOIN ("
